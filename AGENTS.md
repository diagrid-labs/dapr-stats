...
- **A full run is slow by design.** The package loops retry up to three times with a five-minute backoff between attempts, so collection can take ~25 minutes. The CI job polls with a 40-minute deadline inside a 45-minute job timeout; anything that lengthens the retry path needs both raised, and the poll deadline must stay the one that gives up first. The job timeout *cancels* the run, which kills the `if: always()` cleanup and log-dump steps and leaves nothing to diagnose; the poll deadline *fails* it, which does not.
